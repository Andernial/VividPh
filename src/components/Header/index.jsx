...
function Header() {
    ...
    return (
        <div className={`fixed w-full z-30 text-white transition-colors duration-700 ease-in-out md:bg-transparent ${navOpen ? 'h-screen bg-slate-600' : 'bg-transparent'}`}>
        <div className="p-5 flex flex-col w-full h-full md:flex-row md:justify-around ">
          ...
          <nav className={`flex items-center justify-center md:block transition-all duration-700 ease-in-out md:visible md:opacity-100 md:h-auto' ${navOpen ? 'h-screen opacity-100' : 'h-0 opacity-0 invisible'}`}>
